...
const getProduct = async (i) => {
  ...
  const response = await fetch(
    `https://dummyjson.com/products?limit=${size}&skip=${skip}`
  );
  const data = await response.json();
...
const displayProduct = (data, i = 0) => {
  ...
  data.forEach((element, index) => {
   ...
   <td class="fw-bold pt-4"><button class="rounded-circle btn"><a href="updating.html?id=${
     element.id
   }"><img src="assest/img/edit.png" alt="" width="30px" height="30px"></a></button></td>
   <td class="fw-bold pt-4"><button class="rounded-circle btn" onclick = "del_Product(${
     element.id
...
const searchInput = async () => {
  if (seachPro.value != "") {
    ...
    const response = await fetch(
      `https://dummyjson.com/products/search?q=${seachPro.value}`
    );
    const data = await response.json();
    console.log(data);
